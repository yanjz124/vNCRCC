...
class VatsimClient:
    ...
    def __init__(self, url: str, interval: int = 15):
        ...
        self._data_was_new: bool = True  # Whether last fetch returned new data
        ...
        async with self._session.get(self.base_url) as resp:
            ...
            async with self._lock:
                ...
                if vatsim_update_ts is not None:
                    self._data_was_new = (self._vatsim_update_ts != vatsim_update_ts)
    ...
    def _calculate_adaptive_sleep(self) -> float:
        ...
        - When data was unchanged (duplicate), retry quickly (3-5s) to catch next update
        - When data was new, sleep until the next expected update + small offset
        """
        self._resync_counter += 1

        # Quick retry when we got duplicate data — VATSIM hasn't updated yet
        if not self._data_was_new and self._vatsim_update_ts is not None:
            retry = 3.0 + (self._resync_counter % 3) * 1.0
            logger.debug("Data unchanged, quick retry in %.1fs", retry)
            return retry
        ...
        # Clamp: allow faster retry (3s min) to catch updates sooner
        target_sleep = max(3.0, min(20.0, target_sleep))

        logger.debug("Adaptive sleep: %.1fs (cycle pos: %.1fs, offset: %.1fs)",
                      target_sleep, seconds_into_cycle, self._sync_offset)
