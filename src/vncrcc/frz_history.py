...
HISTORY_PATH = Path.cwd() / "data" / "frz_history.json"
...
def _atomic_write(data: Dict[str, Any]):
    ...
    try:
        ...
        print(f"FRZ history written to {HISTORY_PATH}")
    except Exception as e:
        print(f"Error writing FRZ history: {e}")
...
def update_history(cid: str, position: Dict[str, Any]) -> None:
    ...
    print(f"Updated FRZ history for {cid}: now {len(history[cid])} positions")
    _atomic_write(data)